// ============================================================================
// OpenGL 8.1 - 阴影映射（Shadow Mapping）示例
// 教材：第 8 章 Program 1（Prog8_1_shadowMapping）
//
// 本程序演示：
//   1. 两遍渲染（Two-Pass Rendering）
//      Pass 1：从光源视角渲染深度到阴影贴图（Shadow Map）
//      Pass 2：从相机视角渲染场景，采样阴影贴图决定像素是否在阴影中
//   2. 深度纹理 + 帧缓冲对象（FBO）
//   3. sampler2DShadow + textureProj 进行阴影比较
//
// 场景：青铜圆环 + 金色金字塔，固定位置光源产生阴影
//
// 阴影矩阵 b：将 NDC 坐标 [-1,1] 映射到纹理坐标 [0,1]
// ============================================================================
...
// Pass 2：ADS 光照 + textureProj 阴影测试
void passTwo(void) {
...
int main(void) {
	...
	GLFWwindow* window = glfwCreateWindow(1600, 800, "Chapter8 - program1", NULL, NULL);
